...
// Modified from original react-xr since we want the models to be on another layer to prevent raycasting, only changed import of XRControllerModelFactory, I think...
...
export function DefaultXRControllers({ rayMaterial = {} }: { rayMaterial?: MeshBasicMaterialParameters }) {
  ...
  const [rays] = useState(new Map<number, Mesh>())
  ...
  useEffect(() => {
    ...
    controllers.forEach(({ controller, grip, inputSource }) => {
      ...
      ray.layers.set(3) // Prevent ray from being raycasted
