...
import { LinearEncoding, sRGBEncoding, Texture, TextureEncoding } from 'three'
...
const ImageWithUrl = React.forwardRef(({ url, encoding=sRGBEncoding, ...props }: ImageProps, ref: React.ForwardedRef<THREE.Mesh>) => {
  const encodingSetter = useCallback((textures: Texture | Texture[]) => {
    if (textures instanceof Texture) {
      textures.encoding = encoding
    } else {
      textures.forEach((texture) => (texture.encoding = encoding))
    }
  }, [])
  const texture = useTexture(url!, encodingSetter)
