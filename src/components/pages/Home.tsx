...
import { ApplySettings, DisableRender, FadeFilter, TitleScreen, Screen1, Screen2, EndScreen, EndScreenB } from './Home/HomeSubcomponents'
            ...
            <Viewer className="absolute w-full h-full overflow-hidden flex-grow rounded-3xl" defaultCameraProps={{
                ...
                <ScrollControls pages={scrollPages} >
                    ...
                    <Scroll html className="w-full h-full">
                        <FadeFilter pages={scrollPages} onFullyOpaque={()=>{setOpaque(true)}} onNotFullyOpaque={()=>{setOpaque(false)}}>
                            <TitleScreen lang={settings.lang} text={text}></TitleScreen>
                            <Screen1 text={text}></Screen1>
                            <Screen2 text={text}></Screen2>
                            {
                                (posts && posts?.length > 0) ? <EndScreen gotoBrowse={gotoBrowse}/> : <EndScreenB/>
                            }
                        </FadeFilter>
                    </Scroll>
                </ScrollControls>
